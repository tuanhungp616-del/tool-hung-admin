...
# 🌐 KHO CHỨA API NHÀ CÁI (DỄ DÀNG THAY THẾ KHI BỊ DIE LINK)
API_LINKS = {
...
def phan_tich_ai_v6_quantum(kq_list):
    ...
    if len(kq_list) < 10: return {"du_doan": "LOADING CORE...", "ti_le": 0, "tong_tai": tong_tai, "tong_xiu": tong_xiu}
...
async def scan_game(tool: str, key: str):
    ...
    if not url: return {"status": "error", "msg": "Mã Game không hợp lệ hoặc chưa hỗ trợ!"}

    try:
        res = requests.get(url, headers={"User-Agent": "Mozilla/5.0"}, timeout=5).json()
        
        # BỘ LỌC ĐA NĂNG (Xử lý mọi loại định dạng API)
        ...
        # Đảm bảo danh sách mượt mà, lấy 20 phiên mới nhất
        lst = lst[::-1] if (isinstance(lst[0], dict) and lst[0].get("id", 0) < lst[-1].get("id", 0)) else lst
        ...
        for s in lst:
            val = str(s).upper()
            if is_xocdia:
                # XÓC ĐĨA: Ánh xạ Chẵn -> Tài, Lẻ -> Xỉu để lừa con AI V6
